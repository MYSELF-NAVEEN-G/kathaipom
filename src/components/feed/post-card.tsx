...
import type { EnrichedStory, User } from '@/lib/types';
...
function CommentForm({ postId }: { postId: string }) {
  ...
  const handleCommentSubmit = async (formData: FormData) => {
    ...
    // The server action 'addComment' will get the user from the session.
  ...
  return (
    <form
      ...
      className="flex items-center gap-2 px-4 pb-2 pt-2 w-full"
    >
...
export function PostCard({
  ...
  post: EnrichedStory & { reason?: string };
}) {
  const authorName = story.author?.name || 'Unknown Author';
  const authorUsername = story.author?.username || 'unknown';
  const authorAvatar = story.author?.avatar?.imageUrl || '';

  const firstPageContent = Array.isArray(story.content)
    ? story.content[0]
    : story.content;
            ...
            <div className="space-y-2 max-h-24 overflow-y-auto">
              {story.comments.map((comment) => (
                <div key={comment.id}>
                  ...
                  <p className="text-muted-foreground d-inline ml-2">
                    {comment.content}
                  </p>
                </div>
              ))}
